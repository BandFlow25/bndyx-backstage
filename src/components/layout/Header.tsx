...
const Header: React.FC<{ toggleSidebar?: () => void, sidebarOpen?: boolean }> = ({ toggleSidebar, sidebarOpen }) => {
  ...
  const logUI = (message: string, ...args: any[]) => {
    console.log(`UI_FLOW: ${message}`, ...args);
  };
        ...
        <div className="flex justify-between items-center h-16">
          ...
          {currentUser && (
            <nav className="hidden md:flex items-center space-x-1">
              <Link 
                href="/dashboard" 
                className="px-3 py-2 rounded-md text-sm font-medium text-slate-700 dark:text-slate-300 hover:text-orange-500 dark:hover:text-orange-400"
              >
                Dashboard
              </Link>
              <Link 
          ...
          <div className="flex items-center">
            ...
            <button
              onClick={toggleTheme}
              className="p-2 rounded-full text-slate-400 hover:text-slate-200 focus:outline-none"
      ...
      {isMenuOpen && currentUser && (
        <div className="md:hidden border-t border-slate-700">
          <div className="pt-2 pb-3 space-y-1 px-4">
            <Link 
              href="/dashboard" 
              className="block px-3 py-2 rounded-md text-base font-medium text-slate-700 dark:text-slate-300 hover:text-orange-500 dark:hover:text-orange-400"
            ...
            {/* Artist Selection in Mobile Menu */}
            {currentUserArtists.length > 0 && (
              <div className="border-t border-slate-200 dark:border-slate-700 pt-2 mt-2">
                ...
                {currentUserArtists.map(artist => (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 rounded-md text-base font-medium ${currentArtist?.id === artist.id 
                      ? 'bg-orange-100 dark:bg-orange-900/30 text-orange-800 dark:text-orange-300' 
                      : 'text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700'}`}
                  >
                    <div className="flex items-center">
                ...
                {currentArtist && (
                  <button
                    ...
                    className="w-full text-left px-3 py-2 rounded-md text-base font-medium text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700"
                  >
                    <div className="flex items-center">
                      <LogOut size={16} className="mr-2" />
                      <span>Clear Artist Selection</span>
                    </div>
                  </button>
                )}
              </div>
            ...
            {hasAdminRole && (
              <Link 
                href="/admin"
                className="px-3 py-2 rounded-md text-base font-medium text-orange-500 hover:text-orange-600 flex items-center"
                onClick={() => setIsMenuOpen(false)}
              >
